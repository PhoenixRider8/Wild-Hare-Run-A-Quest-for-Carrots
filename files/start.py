...
class Start(object):
    ...
    carrots=[Carrot(256,350,32,32),Carrot(356,450,32,32),Carrot(656,450,32,32)]
    wolves=[Enemy(656,550,45,30,756)]
    gate=Door(1228,500,72,83,0)
        ...
        while running:
            ...
            for wolf in self.wolves:
                if wolf.rect.colliderect(self.rabbit.rect.x, self.rabbit.rect.y+self.rabbit.vel, self.rabbit.width, self.rabbit.height):
                    ...
                    for wolf in self.wolves:
                        wolf.reset(656,550,45,30,756)
                    self.carrots=[Carrot(256,350,32,32),Carrot(356,450,32,32),Carrot(656,450,32,32)]
                    self.rabbit.collectable.val=0
                    self.gate=Door(1228,500,72,83,0)

                    pygame.mixer.music.play(-1)
            
            #check collision for door
            if self.rabbit.rect.x>self.gate.x and self.rabbit.rect.x<self.gate.x+self.gate.width:
                if self.gate.passThrough(self.rabbit) and self.isNext==False:
                    #print("Next level")
                    self.isNext=True
                    self.nextLevel.main()
                    pygame.display.quit()
